                        ...
                        <form method="post" action="{{ route('quizzes.update', $quiz) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('NName') }}" value="{{ $quiz->name }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('course_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="course_id" required class="form-control">
                                        @foreach(\App\Course::orderBy('id', 'desc')->get() as $course)
                                            <option <?php if ($quiz->course->id == $course->id) echo 'selected'?> value="{{ $course->id }}">{{ \Str::limit($course->title, 50) }}</option>
                                        @endforeach
                                    </select>
